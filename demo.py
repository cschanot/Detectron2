    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            if args.output:
                ...
                if num_instances > 0:
                    labels = _create_text_labels(classes, scores, demo.metadata.get("thing_classes", None))
                if cv2.waitKey(0) == 27:
                    break  # esc to quit
    elif args.webcam:
...
if num_instances > 0:
    ...
    labels = _create_text_labels(classes, predictions["instances"].scores, demo.metadata.get("thing_classes", None))
    print(labels)
    matchers = ['person']
